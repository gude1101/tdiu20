...
int const * Sorted_list::Node::value_or_null(Node const * const n) {
  if (n == nullptr) {
    return nullptr;
  } else {
    return &(n->value);
...
Sorted_list& Sorted_list::operator=(Sorted_list const & that) {
	// Self assignment
	if (this == &that) {
		Sorted_list temp = that;
		return *this = temp;
	}

	clear();
	// Loopar baklänges för prestanda
	for (auto it = that._sentinel->previous; it != nullptr; it = it->previous) {
		add(it->value);
	}
...
Sorted_list& Sorted_list::operator=(Sorted_list && that) {
	...
	std::swap(_sentinel, that._sentinel);
...
int const * Sorted_list::first() const {
  return Node::value_or_null(_first);
}

int const * Sorted_list::last() const {
...
bool Sorted_list::Node::is_in_order(Sorted_list::Node const * const a, Sorted_list::Node const * const b) {
  if (b == nullptr) {
    return false;
  }
  if (a == nullptr) {
...
std::ostream& operator<<(ostream& os, Sorted_list const& sl) {
  os << sl.to_string();
  return os;
}
...
string Sorted_list::to_string() const {
  ...
  if (!is_empty()) {
    ss << *first();
    bool one_element = _first->next == _sentinel;
...
bool Sorted_list::test_for_no_connection(Sorted_list const & a, Sorted_list const & b) {
	auto x = a._first == nullptr ? a._sentinel : a._first;

	for (; x != nullptr; x = x->next) {
		auto y = b._first == nullptr ? b._sentinel : b._first;

		for (; y != nullptr; y = y->next) {
			if (x == y) {
				return false;
			}

			if (y->next == nullptr && y != b._sentinel) {
				return false;
			}
		}

		if (x->next == nullptr && x != a._sentinel) {
			return false;
		}
	}
